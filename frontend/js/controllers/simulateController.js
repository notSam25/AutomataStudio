angular.module('automataApp').controller('SimulateController', function($scope, $routeParams, $interval, AutomataService) {
  $scope.automata = {};
  $scope.input = '';
  $scope.result = null;
  $scope.loading = true;
  ...
  $scope.playAnimation = function() {
    if ($scope.isAnimating || !$scope.result || !$scope.result.steps) return;
  ...
  $scope.getStepDescription = function(step, index) {
    return 'Step ' + index + ': State q' + step.state + ' → Symbol: ' + step.symbol;
  };
